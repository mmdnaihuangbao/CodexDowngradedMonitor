...
**完成态补齐**：响应对象在内存里只活到请求结束，完成那一瞬经常采不到，卡片会一直停在「未完成」。
证据线程每 2 秒对 `status ∈ (queued, in_progress)` 的记录做一次对账（每轮上限 1000 条）：

- 索引里有该响应号的会话记账（`rollout_token_usage`）→ 请求已结束；记账行的写入时刻写成 `completed_at`，
  `duration_seconds` 随之算得出来；
- 库里另有响应把它当 `previous_response_id` 引用 → 请求已结束；这种情况**不写 `completed_at`**（没有可信时间）。

两种都是**单向证据**：命中即已结束，没命中保持原状态——已完成的记录里也只有约 73% / 83% 带这两种证据，
所以缺失不能反推成"没完成"，更不会改成失败或取消。
...
- 疑似样本使用灰色虚线，与真实状态分开。
- 完成那一瞬没采到的记录不再长期卡蓝：见上文「完成态补齐」，证据对账后会补成 `completed` 并恢复颜色。
