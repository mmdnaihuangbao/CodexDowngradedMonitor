...
也可双击根目录 `start-cpp.bat` 或 `start-python.bat`。

| 入口 | 采集实现 | 默认端口 |
|---|---|---|
| `start-cpp.bat` / `--backend cpp` | C++ 常驻辅助进程 | 48778 |
...
- 进程发现改为 `CreateToolhelp32Snapshot` / `Process32FirstW` / `Process32NextW`，查询路径使用 `QueryFullProcessImageNameW`，不再运行 PowerShell 枚举命令。两种后端共用此逻辑。
- 存活检查改用 `GetExitCodeProcess`。旧代码的 `WaitForSingleObject` 要求 `SYNCHRONIZE` 权限，原句柄没有该权限，失败被误当作退出，导致反复重连、反复启动 PowerShell。Python WinAPI 还补齐了 64 位 `argtypes/restype`。
...
python cpp_collector/benchmark.py
# 可选：对指定进程进行有限轮次只读性能测量
python cpp_collector/benchmark.py --pid 12345 --rounds 8
```

测试覆盖：唯一对象跨 1 MiB 边界（无重叠确实漏采）、长输入后字段、相邻/嵌套对象隔离、首请求、模型冲突、实际只读内存读取、存活检查、辅助进程复用和退出、延迟配对、两个后端 HTTP/SSE/诊断/配置/状态与关闭。测试样本只分配并写入自己的测试进程内存，不向被观测应用写内存。
